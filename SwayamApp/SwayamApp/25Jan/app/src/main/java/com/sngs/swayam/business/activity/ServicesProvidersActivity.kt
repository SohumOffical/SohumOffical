...
import com.sngs.swayam.business.adapters.ServicesProviderListAdapter
        ...
        ServiceCall.callGetServiceProviderList(this, auth_id, auth_token, Links.User_Type)
            .enqueue(object : Callback<GetServiceProviderBaseResponse> {
                override fun onResponse(call: Call<GetServiceProviderBaseResponse>, response: Response<GetServiceProviderBaseResponse>) {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        if (success_v?.toInt()==1)
                        {
                            ...
                            service_rv_list.adapter = ServicesProviderListAdapter(Links.service_list, this@ServicesProvidersActivity)
                        }
                        else {
                            Links.snack_bar(this@ServicesProvidersActivity,main_layout,response.body()?.message.toString())
                        }
                    } else {
                        Links.snack_bar(this@ServicesProvidersActivity,main_layout,response.body()?.message.toString())
                    }
                ...
                override fun onFailure(call: Call<GetServiceProviderBaseResponse>, t: Throwable) {
                    loading_layout.setVisibility(View.GONE)
                    Links.snack_bar(this@ServicesProvidersActivity,main_layout,t.message.toString())
                }
            })
    }
}